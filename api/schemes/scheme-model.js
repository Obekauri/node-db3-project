...
async function findById(scheme_id) { // EXERCISE B
  const dataFromSchemes = await db('schemes as sc')
  ...
  const result = {
    scheme_id: dataFromSchemes[0].scheme_id,
    scheme_name: dataFromSchemes[0].scheme_name,
    steps: []
  }

  dataFromSchemes.forEach(element => {
    if(element.step_id){
      result.steps.push({
